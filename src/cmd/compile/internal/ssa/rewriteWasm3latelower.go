...
func rewriteValueWasm3latelower_OpOffPtr(v *Value) bool {
	...
	// cond: base.Type.IsPtr() && base.Type.Elem() != nil && base.Type.Elem().IsStruct() && wasm3IsI64ScalarPtr(t)
	// result: (MakeFieldPtr <t> {base.Type.Elem()} [off] base)
	for {
		t := v.Type
		off := auxIntToInt64(v.AuxInt)
		base := v_0
		if !(base.Type.IsPtr() && base.Type.Elem() != nil && base.Type.Elem().IsStruct() && wasm3IsI64ScalarPtr(t)) {
			break
		}
