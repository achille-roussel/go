...
func CanSSA(t *types.Type) bool {
	...
	case types.TARRAY:
		...
		if t.NumElem() == 1 && buildcfg.GOARCH == "wasm3" && (t.Elem().IsStruct() || t.Elem().IsArray()) {
			// wasm3: a composite-element array lowers to one boxed
			// (array (ref ...)) reference. Keeping a [1]T SSA-able
			// decomposes it to ArrayMake1/ArraySelect, which the boxed
			// model does not lower; keep it in memory (like larger
			// arrays) so element access uses array.get / struct.get and
			// folds to the element-ref + FieldGet path. Surfaced by the
			// autogenerated array equality for internal/abi's [1]Entry
			// cache types.
			return false
		}
		if t.NumElem() <= 1 {
