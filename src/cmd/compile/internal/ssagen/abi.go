...
func (s *SymABIs) GenABIWrappers() {
	...
	for _, fn := range typecheck.Target.Funcs {
		...
		// so ABI0 and ABIInternal collapse to the same shape. The
		// ABI-bridging wrappers generated by makeABIWrapper would use
		// SP-relative spills to bridge between the two, which the
		// wasm3 obj backend can't lower (no Go stack frame in linear
		// memory). Skip wrapper generation; the single ABI is callable
...
func CreateWasmImportWrapper(fn *ir.Func) bool {
	...
	if buildcfg.GOARCH != "wasm" && buildcfg.GOARCH != "wasm3" {
