...
func (s *state) addr(n ir.Node) *ssa.Value {
	...
	case ir.OINDEX:
		...
		if n.X.Type().IsSlice() {
			...
			// wasm3: a struct/string/slice/interface element is a boxed ref
			// in the (array (ref ...)) backing, so &a[i] is that element ref
			// (which IS the *elem interior pointer) obtained via array.get —
			// a later field access or deref folds onto it. Without this the
			// dynamic index produces an unlowered OffPtr (PTR PTR).
			if et := n.X.Type().Elem(); buildcfg.GOARCH == "wasm3" &&
				(et.IsStruct() || et.IsString() || et.IsSlice() || et.IsInterface()) {
				arrRef := s.expr(n.X)
				i := s.expr(n.Index)
...
func (s *state) storeType(t *types.Type, left, right *ssa.Value, skip skipMask, leftIsStmt bool) {
	...
	if buildcfg.GOARCH == "wasm3" && skip == 0 && (t.IsSlice() || t.IsString() || t.IsInterface()) {
		// wasm3 boxes a slice/string/interface as a single WasmGC ref and
		// has no linear write barrier (the host GC traces refs), so the
		// write-barrier-driven decomposition below — which stores the Go
		// ABI components (data ptr, len, cap / itab, data) at linear
		// sub-offsets — is both unnecessary and wrong: the destination
		// field/element holds one boxed ref, not multiple linear words
		// (offset 8 names no wasm3 field). Store the whole boxed ref.
...
// InteriorPtr) on wasm3. Covers integer/bool/float scalars (LoadInterior/
// StoreInterior use array.get/array.set: integers extend to i64, floats
// stay f32/f64) and pointer elements ([]*T: the element is a WasmGC ref,
// loaded as anyref and stored via a nullable ref.cast to the backing's
// element type). Composite element interior pointers are deferred.
