...
const (
	...
	heapAddrBits = (_64bit*(1-goarch.IsWasmFamily)*(1-goos.IsIos*goarch.IsArm64))*48 + (1-_64bit+goarch.IsWasmFamily)*(32-(goarch.IsMips+goarch.IsMipsle)) + 40*goos.IsIos*goarch.IsArm64
	...
	logHeapArenaBytes = (6+20)*(_64bit*(1-goos.IsWindows)*(1-goarch.IsWasmFamily)*(1-goos.IsIos*goarch.IsArm64)) + (2+20)*(_64bit*goos.IsWindows) + (2+20)*(1-_64bit) + (9+10)*goarch.IsWasmFamily + (2+20)*goos.IsIos*goarch.IsArm64
...
// mallocgc lives in mallocgc_default.go (the full per-P cache /
// GC-assisted allocator) and mallocgc_wasm3.go (a one-line
// redirect into the bump-heap newobject). Splitting lets the
// wasm3 build skip the mcache / mheap / fixalloc subgraph, which
// the wasm3 backend can't lower while the M2 cutover is still
// in flight — see doc/wasm3-m3-stage-f-interfaces.md.
