...
// concatstrings is declared per-target: string_concat_default.go
// (!wasm3) holds the canonical rawstringtmp+memmove body;
// string_concat_wasm3.go has a wasm3-only body that bypasses the
// shared []byte intermediate (whose .array is anyref per the wasm3
// slice ABI, incompatible with memmove's i64 pointer parameters).
