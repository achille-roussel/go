...
// memmove is declared per-target: stubs_memmove_default.go on
// every arch other than wasm3 holds the body-less linkname'd
// forward decl (the body is in memmove_*.s); memmove_wasm3.go
// provides a Go-language body since the wasm3 obj backend
// can't lower the asm version's FP-relative arg loads (the
// wasm3 ABI passes args in registers, not in a linear-memory
// stack frame).
